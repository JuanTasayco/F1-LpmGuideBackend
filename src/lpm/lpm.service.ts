  ...
  async updateSectionById(id: string, updateDto: UpdateLpmDto) {
    ...
    try {
      ...
      if (ingreso && contenido) {
        ...
        await queryRunner.manager.delete(LpmContentImagesIngreso, { ingreso: id });
        section.contenido = contenido.map(contenido => this.lpmImageRepository.create({ subtitles: contenido.subtitles, imagesUrl: contenido.imagesUrl }));
        section.ingreso = ingreso.map(ingreso => this.lpmIngresoRepository.create({ subtitles: ingreso.subtitles, imagesUrl: ingreso.imagesUrl }));
